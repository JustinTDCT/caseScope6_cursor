...
def process_file(file_id: int, file_path: str, case_id: int):
    ...
    try:
        ...
        # Clean up temporary files (but keep original file for reindexing)
        # os.remove(file_path)  # Don't delete original file
...
def rerun_rules_for_file(file_id: int, db: Session) -> bool:
    ...
    db = SessionLocal()
    try:
        print(f"DEBUG: rerun_rules_for_file called with file_id={file_id}")
        file_record = db.query(File).filter(File.id == file_id).first()
        if not file_record:
            print(f"DEBUG: File record not found for file_id={file_id}")
            return False
        ...
        if not os.path.exists(file_path):
            print(f"DEBUG: File not found in uploads, checking if it was processed and cleaned up")
            file_record.status = "error"
            file_record.error_message = "Original file not available for re-indexing (was cleaned up after processing)"
            file_record.progress = 100
            db.commit()
            return False
...
def reindex_file(file_id: int, db: Session) -> bool:
    ...
    db = SessionLocal()
    try:
        print(f"DEBUG: reindex_file called with file_id={file_id}")
        file_record = db.query(File).filter(File.id == file_id).first()
        if not file_record:
            print(f"DEBUG: File record not found for file_id={file_id}")
            return False
